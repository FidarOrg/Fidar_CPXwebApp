...
export default function AtmBluetoothGate({ onReady }) {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      console.error("[FIDAR ATM] Bluetooth proximity error:", err);
    ...
    <div className="flex flex-col gap-4 items-center text-center px-4 py-6">
      ...
      {blocked && (
        <p className="text-xs text-muted-foreground max-w-xs">
          Open browser settings → Privacy & Security → Bluetooth → Allow, then
          reload this page.
        </p>
      )}
      ...
      {unsupported && (
        <p className="text-xs text-muted-foreground max-w-xs">
          Please open this page in <span className="font-semibold">Google Chrome</span> or{" "}
